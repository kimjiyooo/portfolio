...
$(document).ready(function() {
    ...
    $('.btn1').css('width','110px'); // 버튼의 너비 증가
    
    $('.gallery .link1').fadeIn('normal'); //첫번째 이미지 보인다..
    $('.gallery .link1 span').delay(1500).animate({top:450, opacity:1},'slow');
    $('.gallery .link1 p').delay(2000).animate({top:610, opacity:1},'slow');
    ...
    //   }
      ...
      $('.mbutton').css('width','60px'); // 버튼 원래의 너비
      $('.btn'+cnt).css('background','#fff');//자신만 불켜
      $('.btn'+cnt).css('width','110px');    

      $('.gallery li span').css('top',300).css('opacity',0); //텍스트의 초기화 (원위치)
      $('.gallery li p').css('top',500).css('opacity',0);
      $('.gallery .link'+cnt).find('span').delay(1000).animate({top:450, opacity:1},'slow');
      $('.gallery .link'+cnt).find('p').delay(1500).animate({top:610, opacity:1},'slow');

       if(cnt==imageCount)cnt=0;  //카운트의 초기화 0
     }
     
    timeonoff= setInterval( moveg , 4500);// 타이머를 동작 1~5이미지를 순서대로 자동 처리
  ...
  $('.ps').click(function(){ 
    ...
    timeonoff= setInterval( moveg , 4500); //부활
